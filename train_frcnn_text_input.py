...
record_path = 'F:\\strawberry_disease_detection\\keras_frcnn\\records_new.csv'

train_path = "F:\\strawberry_disease_detection\\augmentation\\bb_box_seven_class_more_data.txt"
C = config.Config()
C.model_path = 'F:\\strawberry_disease_detection\\keras_frcnn\\model_frcnn.hdf5'
C.num_rois = 32
...
C.base_net_weights = 'C:\\Users\\bhuwan\\Desktop\\resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5' # for image net weight
...
leaf_blight_training = [s for s in train_imgs if s['bboxes'][0]['class'] == 'leaf_blight']
tip_burn_training = [s for s in train_imgs if s['bboxes'][0]['class'] == 'tip_burn']
mildew_training = [s for s in train_imgs if s['bboxes'][0]['class'] == 'mildew']
leaf_spot_training = [s for s in train_imgs if s['bboxes'][0]['class'] == 'leaf_spot']
graymold_training = [s for s in train_imgs if s['bboxes'][0]['class'] == 'graymold']
angular_leaf_spot_training = [s for s in train_imgs if s['bboxes'][0]['class'] == 'angular_leaf_spot']
anthracnose_training = [s for s in train_imgs if s['bboxes'][0]['class'] == 'anthracnose']
...
print('Num leaf blight samples for training {}'.format(len(leaf_blight_training)))
print('Num tip_burn samples for training{}'.format(len(tip_burn_training)))
print('Num mildew samples for training {}'.format(len(mildew_training)))
print('Num leaf_spot samples for training{}'.format(len(leaf_spot_training)))
print('Num graymold samples for training {}'.format(len(graymold_training)))
print('Num angular leaf spot samples for training {}'.format(len(angular_leaf_spot_training)))
print('Num anthracnose samples for training {}'.format(len(anthracnose_training)))
